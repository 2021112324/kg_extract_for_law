...
async def guide_clause_extract_by_dir(
    ...
    """
基于单门的合规指引文件知识图谱抽取流程设计的接口
功能：对指定目录下的合规指引文件进行知识图谱抽取，以指定目录为总图谱kg，每个文件生成一个图谱task，if_del_task设置合并图谱时是否删除task，默认为False
1. 抽取流程核心代码：app/infrastructure/information_extraction/guide_extract
2. 提示词文件位置：app/infrastructure/information_extraction/guide_extract/prompt
...
Raises:
    ...
    try:
        background_tasks.add_task(
            kg_task_manager.run_async_function,
            kg_service.guide_clause_extract_by_local_dir,
            {"guide_clause_file_dir": data_dir, "if_del_task": if_del_task, "db": db}
        )
        return success_response(
            msg="任务开始执行",
            data=None
...
async def litigation_extract_by_dir(
    ...
    """
基于单门的诉讼文书的知识图谱抽取流程设计的接口
功能：对指定目录下的诉讼文书进行知识图谱抽取，以指定目录为总图谱kg，每个文件生成一个图谱task，if_del_task设置合并图谱时是否删除task，默认为False
1. 抽取流程核心代码：app/infrastructure/information_extraction/case_extract/case_extract.py
2. 提示词文件位置：app/infrastructure/information_extraction/case_extract/prompt
...
Raises:
    ...
    try:
        background_tasks.add_task(
            kg_task_manager.run_async_function,
            kg_service.litigation_extract_by_local_dir,
            {"litigation_file_dir": data_dir, "case_type": case_type, "if_del_task": if_del_task, "db": db}
        )
        return success_response(
            msg="任务开始执行",
            data=None
