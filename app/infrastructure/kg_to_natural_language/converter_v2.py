...
def _emit(type_lines: list[str], file_counter: Counter[str], filename: str, line: str) -> None:
    """输出一条知识并统计来源文件条目数。"""
...
def convert_law_articles(
    ...
    for record in node_records:
        ...
        if not is_empty_value(scope):
            _emit(type_lines, file_counter, filename, f"{name}的效力范围是{value_to_text(scope)}")
...
def convert_clause_units(
    ...
    for record in node_records:
        ...
        if not is_empty_value(consequence):
            emit(f"违反{name}规定的，{value_to_text(consequence)}。")
...
def write_txt(type_lines: list[str], graph_type: str) -> Path:
    """将知识条目写入 temp/txt/<graph_type>.txt。"""
    txt_dir = _TEMP_DIR / "txt"
...
def write_csv(type_lines: list[str], graph_type: str, file_stats: dict[str, int]) -> Path:
    """生成统计 CSV，写入 temp/csv/知识条目统计.csv。"""
    csv_dir = _TEMP_DIR / "csv"
...
def convert_json_to_text_v2(graph_type: str) -> dict[str, Any]:
    """
    从 temp/json/ 读取 node.json 和 edge.json，按"过滤+组装"规则转换为
    自然语言知识并输出到 temp/txt/ 和 temp/csv/。
    ...
    convert_law_articles(node_records, type_lines, file_counter)
    convert_clause_units(node_records, type_lines, file_counter)
    convert_references(edge_records, identity_index, element_id_index, type_lines, file_counter)

    # 写入结果
    txt_path = write_txt(type_lines, graph_type)
    file_stats = dict(file_counter)
    csv_path = write_csv(type_lines, graph_type, file_stats)
