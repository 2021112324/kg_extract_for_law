...
FORMAT_THREE_DOCUMENT_FORMAT = "format_three_us_code_cfr_section"
...
# Section 级 LegalProvision 属性抽取的轻量输入阈值。
# 超过字符数阈值时，Section 级 LLM 不再接收完整 Section 原文，只接收开头、结尾和修订关键词附近文本。
SECTION_LIGHT_INPUT_CHAR_THRESHOLD = int(os.getenv("SECTION_LIGHT_INPUT_CHAR_THRESHOLD", "20000"))
SECTION_LIGHT_INPUT_HEAD_CHARS = int(os.getenv("SECTION_LIGHT_INPUT_HEAD_CHARS", "16000"))
SECTION_LIGHT_INPUT_TAIL_CHARS = int(os.getenv("SECTION_LIGHT_INPUT_TAIL_CHARS", "4000"))
SECTION_AMENDMENT_KEYWORD_WINDOW_CHARS = int(os.getenv("SECTION_AMENDMENT_KEYWORD_WINDOW_CHARS", "1200"))
...
DEFAULT_REPORT_DIR = (
    ...
    r"\app\infrastructure\information_extraction\en_law_v3\report"
)
