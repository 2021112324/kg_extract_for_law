...
from app.infrastructure.information_extraction.en_law_v3.config import RISK_TYPE_VALUES
...
def _entity_key(name: str) -> str:
    ...
    value = re.sub(r"\b(?:sections?|sec\.?)\s+", "section ", value, flags=re.IGNORECASE)
    return re.sub(r"\s+", " ", value).lower()


def _section_alias_variants(value: Any) -> list[str]:
    """为格式三 Section / subsection 编号生成常见引用别名。"""

    text = str(value or "").strip()
    if not text:
        return []
    aliases = [text]
    section_match = re.search(r"§\s*(?P<number>\d+[0-9A-Za-z_.-]*)(?P<suffix>(?:\([^)]+\))*)", text)
    if section_match:
        number = section_match.group("number")
        suffix = section_match.group("suffix") or ""
        aliases.extend([
            f"§ {number}{suffix}",
            f"§{number}{suffix}",
            f"section {number}{suffix}",
            f"Section {number}{suffix}",
            f"Sec. {number}{suffix}",
            f"sec. {number}{suffix}",
            f"{number}{suffix}",
        ...
        if suffix:
            ...
            for end in range(len(tokens) - 1, 0, -1):
                parent_suffix = "".join(tokens[:end])
                aliases.extend([
                    f"§ {number}{parent_suffix}",
                    f"§{number}{parent_suffix}",
                    f"section {number}{parent_suffix}",
                    f"Section {number}{parent_suffix}",
                    f"Sec. {number}{parent_suffix}",
                    f"{number}{parent_suffix}",
                ])
            aliases.extend([
                f"§ {number}",
    ...
    return list(dict.fromkeys(alias for alias in aliases if str(alias or "").strip()))
...
def _register_many(lookup: dict[str, str], node_id: str, values: list[Any]) -> None:
    ...
    for value in values:
        ...
        for alias in _section_alias_variants(value):
            _register_lookup(lookup, alias, node_id)
...
def _article_aliases_from_text(text: str) -> list[str]:
    """从文本中解析格式三 `§` / CFR / USC 引用别名。"""
    aliases = []
    value = str(text or "")
    # 支持 § 730.1、§ 1030、§ 730.1(a) 等引用。
    for match in re.finditer(r"§\s*\d+[0-9A-Za-z_.-]*(?:\([^)]+\))*", value):
        aliases.append(match.group(0))
        base = re.match(r"§\s*\d+[0-9A-Za-z_.-]*", match.group(0))
        if base:
            aliases.append(base.group(0))
            aliases.append(base.group(0).replace("§", "").strip())
    # 支持 15 CFR 730.1、22 CFR 120.1、18 U.S.C. 1030 等引用。
    for match in re.finditer(r"\b(?:\d+\s+CFR|\d+\s+U\.?S\.?C\.?)\s+(?P<section>\d+[0-9A-Za-z_.-]*)", value, re.IGNORECASE):
        section = match.group("section")
        aliases.append(section)
        aliases.append(f"§ {section}")
    # 支持 section 730.1、Sec. 101、sections 101(a) 等无 § 符号写法。
    for match in re.finditer(
        r"\b(?:sections?|sec\.?)\s+(?P<section>\d+[0-9A-Za-z_.-]*)(?P<suffix>(?:\([^)]+\))*)",
        value,
        re.IGNORECASE,
    ):
        section = match.group("section")
        suffix = match.group("suffix") or ""
        aliases.extend(_section_alias_variants(f"§ {section}{suffix}"))
    return aliases
        ...
        for context in contexts:
            ...
            for provision_clause in article.get("provision_clauses") or []:
                ...
                for name, props in citation_candidates:
                    ...
                    if target_id:
                        _register_many(node_lookup, target_id, _citation_aliases(name, props))
                        source_id = text_paragraph_nodes[0]["node_id"] if text_paragraph_nodes else clause_node["node_id"]
                        _append_edge(
                            edges,
                            edge_seen,
                            make_edge(source_id, target_id, "CITES", filename, {"source": "resolved_internal_citation"}),
                        )
                        continue
                    citation_key = _citation_key(name, props)
                    node = external_citation_lookup.get(citation_key)
