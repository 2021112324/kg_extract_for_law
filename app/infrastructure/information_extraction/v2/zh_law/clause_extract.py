...
from app.infrastructure.information_extraction.v2.zh_law.config import DEFAULT_CONFIG, ZhLawConfig
from app.infrastructure.information_extraction.v2.zh_law.io_utils import load_text, output_path, save_json
from app.infrastructure.information_extraction.v2.zh_law.prompt.example import example_for_clause, \
    example_for_file_info
from app.infrastructure.information_extraction.v2.zh_law.prompt.prompt import prompt_for_clause, prompt_for_file_info
from app.infrastructure.information_extraction.v2.zh_law.prompt.schema import schema_for_clause, schema_for_file_info
from app.infrastructure.information_extraction.v2.zh_law.stats import ResultStats
...
class ClauseExtractor:
    def __init__(
            ...
            config: ZhLawConfig | None = None,
            extractor: Any | None = None,
            lenient_mode: bool | None = None,
    ):
        ...
        self.extractor = extractor or InformationExtractionFactory.create(
            "langextract", max_retries=self.config.max_retries, config=self.extractor_config
        )
        self.semaphore = asyncio.Semaphore(max_concurrent or self.config.max_concurrent)
        self.result_stats = ResultStats()
        self.lenient_mode = self.config.lenient_mode if lenient_mode is None else lenient_mode
        self.last_split_result: dict = {}
        self.last_extraction_status: dict = {}
    ...
    async def extract_clauses(
        ...
        """Extract a Chinese regulation while retaining the V1 public API."""
        self.result_stats = ResultStats(total_files=1)
        ...
        try:
            ...
            self.last_split_result = clauses_data
            logging.info("📄:结束分割条款")
            ...
            for i, result in enumerate(results):
                ...
                elif not isinstance(result, dict) or not result.get("node_id"):
                    message = f"条款 {i + 1} 返回空或不完整抽取结果"
                    logging.error(message)
                    self.result_stats.add_extraction_error(message)
                    failed_results.append((i + 1, ValueError(message)))
                    failed_clauses.append(clauses[i])
                else:
            ...
            self.last_extraction_status = {
                "filename": filename,
                "status": "incomplete" if failed_results else "complete",
                "source_clause_count": len(clauses),
                "successful_clause_count": len(successful_results),
            ...
            if failed_results:
                self.result_stats.add_strong_warning(
                    f"宽松模式生成不完整结果，失败法条数: {len(failed_results)}"
                )
            ...
            if not failed_results:
                self.result_stats.success_files = 1
            final_kg["metadata"] = {
                **self.last_extraction_status,
                "model_success": not failed_results,
                "run_stats": self.result_stats.to_dict(),
            }
            return final_kg
        except Exception as e:
            logging.error("📄❌：条款知识图谱抽取报错: %s", e)
            self.last_extraction_status["status"] = "failed"
            self.result_stats.add_file_processing_error(f"{filename}: {e}")
            raise
...
4. 条款内容为上一个"第X条"到下一个"第X条"之间的内容，或者读到章或节的标志，或者读到文件末尾，或者读到"附录"、"附件"等条款部分结束标志。
    ...
    async def kg_extract_from_file_info(
        ...
        try:
            ...
            if not file_info_processed:
                raise ValueError("模型结果中缺少有效的法规文件实体")
            clause_cache.file_info = file_info_result
            return file_info_result
        except Exception as e:
            logging.error(f"📄❌：抽取文件信息时出错 - {e}")
            self.result_stats.add_extraction_error(
                f"抽取文件信息时出错 - {e}; input={file_info[:500]}"
            )
            raise ValueError(f"抽取文件信息时出错 - {e}") from e
    ...
    async def kg_extract_from_clause(
        ...
        async with self.semaphore:
            try:
                ...
                if not clause_processed:
                    raise ValueError(f"模型结果中缺少有效法条实体: {clause_number}")
                clause_cache.clause_cache[clause_number] = clause_result
                return clause_result
            except Exception as e:
                logging.error(f"📄❌：处理文件{filename}时出错 - {e}")
                self.result_stats.add_extraction_error(
                    f"处理文件{filename}法条{one_clause.get('条款编号', '')}时出错 - {e}"
                )
                raise ValueError(
                    f"处理文件{filename}法条{one_clause.get('条款编号', '')}时出错 - {e}"
                ) from e
    ...
    async def _save_cache_to_json(
        ...
        """
        target = Path(output_dir) / f"{Path(filename).stem}_cache.json"
        return save_json(
            {
    ...
    async def _load_cache_from_json(
        ...
        cache = ClauseCache()
        cache.file_info = payload.get("file_info") or {}
        cache.clause_cache = payload.get("clause_cache") or {}
        return cache

    async def extract_text_to_kg(self, filename: str, text: str) -> dict:
        """V2 semantic alias for the V1-compatible extraction entry point."""
        return await self.extract_clauses(filename=filename, text=text)

    async def extract_file_to_kg(self, input_path: str | Path, output_dir: str | Path) -> dict:
        """Extract one UTF-8 text/Markdown regulation and save auditable stages."""
        source = Path(input_path)
        text = load_text(source)
        try:
            kg = await self.extract_clauses(filename=source.name, text=text)
        except Exception:
            save_json(self.last_split_result, output_path(source, output_dir, "split"))
            save_json(self.last_extraction_status, output_path(source, output_dir, "extraction"))
            save_json(self.result_stats.to_dict(), output_path(source, output_dir, "summary"))
            raise

        paths = {
            "split": save_json(self.last_split_result, output_path(source, output_dir, "split")),
            "extraction": save_json(
                self.last_extraction_status,
                output_path(source, output_dir, "extraction"),
            ),
            "summary": save_json(
                self.result_stats.to_dict(),
                output_path(source, output_dir, "summary"),
            ),
        }
        kg["output_paths"] = {**paths, "kg": str(output_path(source, output_dir, "kg"))}
        save_json(kg, output_path(source, output_dir, "kg"))
        return kg

    def get_run_summary(self) -> dict:
        return self.result_stats.to_dict()
    ...
    async def logging_result_stats(self):
        ...
        # logging.info("======================================================================")
        logging.info("📄✅：数据统计")
        logging.info(f"📄✅：文件数: {self.result_stats.total_files}")
        logging.info(f"📄✅：成功文件数: {self.result_stats.success_files}")
        logging.info(f"📄✅：错误数: {self.result_stats.error}")
        logging.info(f"📄✅：抽取错误数: {self.result_stats.extraction_error}")
        logging.info(f"📄✅：文件处理错误数: {self.result_stats.file_processing_error}")
        logging.info(f"📄✅：弱警告数: {self.result_stats.week_warning}")
        logging.info(f"📄✅：强警告数: {self.result_stats.strong_warning}")


ZhLawExtractor = ClauseExtractor
