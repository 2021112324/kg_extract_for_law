...
class LegalDocumentAIChunker:
    ...
    async def chunk_legal_document(
        ...
        logging.error("📝错误：诉讼文书分块时，所有抽取尝试都失败了")
    ...
    def _parse_chunking_result(
        ...
        try:
            ...
            logging.warning(f"📝⚠️警告：无法解析JSON格式的结果: {chunk_result[:100]}..., 尝试修复json")
...
# 4. 当文件过于复杂时，只需要确保分块内容覆盖了原始文档中的重要内容。
        try:
            ...
            if "是" in result or "yes" in result:
                ...
                logging.warning(f"📝⚠️警告：大模型响应不明确: {result}")
                return False

        except Exception as e:
            logging.error(f"📝❌错误：调用大模型验证分块覆盖原文时发生错误: {str(e)}")
            return False
