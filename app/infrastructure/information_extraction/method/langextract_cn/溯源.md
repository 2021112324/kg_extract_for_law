实体溯源功能代码分析
1. 整体概述
在LangExtract库的resolver模块中，align_extractions和_fuzzy_align_extraction两个函数共同实现了实体溯源功能。
2. 实体溯源是指将从文本中提取的实体与其在原始文档中的确切位置进行关联的过程，这使得我们不仅能知道提取了什么实体，
3. 还能知道它在原文档中的具体位置。
2. 代码实现逻辑
...
这种设计提高了实体溯源的准确性和鲁棒性，即使提取的实体与原文本存在细微差异（如大小写、单复数、标点符号等），
也能正确关联到原始位置。
4. 关键特性
容错性：通过模糊匹配算法处理文本变体
