...
class PromptTemplateStructured:
  """A structured prompt.txt template for few-shot examples.
...
def read_prompt_template_structured_from_file(
  ...
  """Reads a structured prompt.txt template from a file.
  ...
  try:
    ...
    raise ParseError(
        f"Failed to parse prompt.txt template from file: {prompt_path}"
    ) from e
...
class QAPromptGenerator:
  ...
  def __str__(self) -> str:
    """Returns a string representation of the prompt.txt with an empty question."""
    return self.render("")

  def format_example_as_text(self, example: data.ExampleData) -> str:
    """Formats a single example for the prompt.txt.
  ...
  def render(self, question: str, additional_context: str | None = None) -> str:
    """Generate a text representation of the prompt.txt.

    Args:
      question: That will be presented to the model.
      additional_context: Additional context to include in the prompt.txt. An empty
        string is ignored.

    Returns:
      Text prompt.txt with a question to be presented to a language model.
    """
